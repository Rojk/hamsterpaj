...
{
	var reply_instructions = 'Här kan du svara på kommentaren =)';
	...
	{
		xmlhttp_ping('/ajax_gateways/comments.php?action=comment_answer&id=' + comment_id + '&reply=' + reply);
		loadFragmentInToElementByPOST('/ajax_gateways/comments.php', 'comments_list',
									"update=true" +
									"&item_id=" + document.getElementById('comment_item_id').value + 
									"&item_type=" + document.getElementById('comment_item_type').value + 
									"&return_list=1");
		return false;
	}
}
...
{
	//popup window
	popup = window.open("http://www.hamsterpaj.net/ajax_gateways/comments.php" + 
					"?action=comments_list_all" +
					"&item_id=" + document.getElementById('comment_item_id').value + 
					"&item_type=" + document.getElementById('comment_item_type').value,
				"Alla kommentarer",
				"width=600,height=600");
	popup.moveTo(170, 100);
	return false;
}
...
function comment_remove(id, comment_owner)
{
	if(confirm('Är du säker på att du vill radera kommentaren?'))
	{
		...
		xmlhttp.open('GET', '/ajax_gateways/comments.php?action=comment_remove&id='+id, true);
...
womAdd('comment_submit_click_enable()');
womAdd('comments_view_all_button_click_enable()');
