function checkChosenCategory(){
	if (document.getElementById('sex_category').value == 'X')
	{
		alert('Du glömde att välja en kategori.');
		return false;
	}
	else if (document.getElementById('title').value == '')
	{
		alert('Du glömde att fylla i en titel');
		return false;
	}
	else
	{
...
function sex_sense_init()
{
	...
	$('.sex_sense_post_header').click(function()
	{
		$('.hidden_content').hide('slow');
		$('.content').hide('slow');
		
		var elementname = $('#content_' + new String($(this).attr('id')).split('_')[1]);
