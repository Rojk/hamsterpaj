function enable_flags_customize_navigation()
{
	...
	{
		var categories = document.getElementById('flags_customize_navigation').childNodes;
...
{
	var flags_nav_chosen_tab;
	var hide_categories = getElementsByClassName(document, 'div', 'flags_customize_category');
	for(var i = 0; i < hide_categories.length; i++)
	{
...
function flags_customize_submit(form_id)
{
	...
	xmlhttp_ping('/traffa/flags_ajax_gateway.php?action=update' + querystring);
	//window.location = '/traffa/flags_ajax_gateway.php?action=update' + querystring;
	document.getElementById('flags_customize_message').innerHTML = 'Dina flaggor har sparats!';
	setTimeout("clear_customize_message();", 1500)
}
