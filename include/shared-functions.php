...
/*		
	...
	function read_only($user)
	{
	$query = 'SELECT userid FROM forum_read_only WHERE userid = "' . $user . '" AND expire > UNIX_TIMESTAMP()';
...
function clickable_links($str)
{
	...
	// Leta upp länkar
	$str = preg_replace('#((http://|ftp://|www\.)(www\.)?)([a-z0-9\#_%\?&//=\+@\.:;-]{4,})#eis', 
						"
						('$2' != 'http://' && '$2' != 'ftp://') ? 
