...
*/
			...
			$output .= '<ul class="avatar_christmas_poll">'."\n";
				while($data = mysql_fetch_assoc($result))
				{
					$output .= "\t".'<li>'."\n";
							...
							$output .= "\t".'<br />'."\n";
							$output .= 'VÃ¤lj <input type="radio" value="'.$data['contender_id'].'" name="avatar_christmas_id" />'."\n";
	...
	function christmas_avatar_admin_add()
	{
		...
		$output .= '<form action="/ajax_gateways/christmas_avatars_poll.php?action=admin_add_vote" method="post" id="christmas_avatar_form">'."\n";
			...
			$output .= '<label for="poll_publish">Publiseringsdatum:</label> <input type="text" value="'.date('Y/m/d H:i:s').'" name="poll_publish" id="poll_publish" /> (frÃ¥n nÃ¤r man ska kunna rÃ¶sta. FÃ¥r <strong>inte</strong> vara <strong>efter utgÃ¥ngsdatumet</strong>!)<br />'."\n";
			$output .= 'SlÃ¤pp direkt: <input type="checkbox" name="release_immediately" checked="checked" /> - Just nu slÃ¤pps allt direkt eftersom den schemalagda funktionen inte Ã¤r klar Ã¤n.<br /><br />'."\n";
			$output .= '<label for="poll_expire">UtgÃ¥ngsdatum:</label> <input type="text" value="'.date('Y/m/d H:i:s', time()+60*60*24*4).'" name="poll_expire" id="poll_expire" /> (Efter detta datum kan man inte rÃ¶sta. FÃ¥r <strong>inte</strong> vara <strong>fÃ¶re publiseringsdatumet</strong>!)<br />'."\n";
