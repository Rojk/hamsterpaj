...
function plump_combinations($dices)
{
	...
	{
		if($result > 0 && is_int($result) && $result < 43)
		{
...
function plump_pos_by_number($number)
{
	...
	$return['col'] = $number - (($return['row']-1) *7);
...
function plump_square_free($number)
{
	...
	if($_SESSION['plump']['board'][$pos['row']][$pos['col']] == 1 || $number > 42)
	{
		return false;
	}
	else
...
function plump_draw_board()
{
	...
	for($row = 1; $row < 7; $row++)
	{
		...
		for($col = 1; $col <= 7; $col++)
		{
			...
			{
				echo '		<td class="vacant"><a href="?action=mark&amp;number=' . ((($row-1)*7) + $col) . '">' . ((($row-1)*7) + $col) . '</a></td>' . "\n";
			}
			else
			{
				echo '		<td class="occupied">' . ((($row-1)*7) + $col) . '</td>' . "\n";
			}
		}
		echo '	</tr>' . "\n";
	}
