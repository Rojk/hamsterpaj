...
//	require('include/core/common.php');
	...
	$output .= '<a href="http://www.hamsterpaj.net/filmklipp/jakten_paa_abdullah_mohammed.html">' . "\n";
	$output .= (date('m-d') == '08-14') ? '<img style="margin-top: 5px;" src="http://images.hamsterpaj.net/tjuvpolis.png" />' : '';
	$output .= '</a>' . "\n";
...
*/
	...
	$output .= '<a href="/traffa/new_photos.php">Fler foton &raquo;</a>&nbsp;<a href="/traffa/photos.php">Ladda upp egna foton &raquo;</a>' . "\n";
	}
	$short_months = array('Jan', 'Feb', 'Mar', 'Apr', 'Maj', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dec');	
	$events = query_cache(array('query' => 'SELECT * FROM recent_updates WHERE label NOT LIKE "Generalrepetition" ORDER BY id DESC LIMIT 12'));
	...
	Heggan - [Hey, stop stealing monkeys] säger (21:16):
	modulen på förstasidan
	Heggan - [Hey, stop stealing monkeys] säger (21:16):
	den måste bort
	Joel säger (21:16):
	*döda*
	...
	mysql lodar typ 98
	Heggan - [Hey, stop stealing monkeys] säger (21:16):
	och spottar SELECT up.*, l.username FROM user_photos AS up, login AS l WHERE l.id = up.user AND up.deleted = 0 AND l.username NOT LIKE "borttagen" 		ORDER BY up.id DESC LIMIT 0, 4;

	Heggan - [Hey, stop stealing monkeys] säger (21:16):
	sådär 4 gånger i sekunden <3
	Heggan - [Hey, stop stealing monkeys] säger (21:17):
	varje query tar ca 10-15 sekunder att köra 

	
	$query = 'SELECT l.id, l.username, u.gender, u.birthday, z.spot, GROUP_CONCAT(DISTINCT uf.flag) AS flags';
	$query .= ' FROM login AS l, userinfo AS u, zip_codes AS z, user_flags AS uf';
	$query .= ' WHERE u.userid = l.id AND u.image = 2 AND u.gender = "%GENDER%" AND z.zip_code = u.zip_code AND uf.user = l.id';
	$query .= ' GROUP BY l.id ORDER BY l.lastlogon DESC LIMIT 7';
	
	$girl_query = str_replace('%GENDER%', 'f', $query);
	$boy_query = str_replace('%GENDER%', 'm', $query);

	$girls = query_cache(array('query' => $girl_query, 'max_delay' => 30));
	$boys = query_cache(array('query' => $boy_query, 'max_delay' => 30));
	
	// Show girls first as default, but if user is logged on and has set gender = f, show boys first.
	$users = ((login_checklogin() && $_SESSION['userinfo']['gender'] == 'f') ? array_merge($boys, $girls) : array_merge($girls, $boys));

	foreach($users AS $key => $user)
	{
		$users[$key]['flags'] = explode(',', $user['flags']);
	}

	$flags_query = 'SELECT id, handle FROM user_flags_list ORDER BY id ASC';
	$flags = query_cache(array('query' => $flags_query, 'max_delay' => 3600, 'category' => 'user_flags'));
	foreach($flags AS $flag)
	{
		$flags_by_id[$flag['id']] = $flag['handle'];
	}*/
