<?php
	...
	function render_friends_list($friends, $params = array())
	{
		...
		foreach($friends AS $friend)
		{
			...
			{
				$return .= '<a class="relation_end_control" href="/traffa/friends.php?id=' . $friend['user_id'] . '&action=removefriend">[Avsluta relationen]</a>' . "\n";
			}
			$return .= '</li>' . "\n";
		}
		$return .= '</ul>' . "\n";
	...
	$output .= '<div class="profile_' . $profile['profile_theme'] . '">' . "\n";
	...
	if(login_checklogin() && $_GET['action'] == 'addfriend' && $_GET['id'] != $_SESSION['login']['id'])
	{
		...
		$output .= '<form action="/traffa/guestbook.php?action=send_new_message&userid=' . $_GET['id'] . '" method="post">' . "\n";
	...
	if(login_checklogin() && $_GET['action'] == 'removefriend' && $_GET['id'] != $_SESSION['login']['id'])
	{
		$query = 'DELETE FROM friendslist WHERE user_id = "' . $_SESSION['login']['id'] . '" AND friend_id = "' . $_GET['id'] . '" LIMIT 1';
		mysql_query($query);		
	}
	...
	if(login_checklogin() && $_GET['id'] != $_SESSION['login']['id'])
	{
		if(!isset($friends[$_SESSION['login']['id']]) && !isset($fans[$_SESSION['login']['id']]))
		{
			$output .= rounded_corners_top(array('color' => 'blue_deluxe'), true);
			$output .= $user['username'] . ' ligger inte på din vännerlista, <a href="?id=' . $_GET['id'] . '&action=addfriend">lägg till</a>';
			$output .= rounded_corners_bottom(array('color' => 'blue_deluxe'), true);
		}
		if(isset($friends[$_SESSION['login']['id']]) || isset($fans[$_SESSION['login']['id']]))
		{
			$output .= rounded_corners_top(array('color' => 'blue_deluxe'), true);
			$output .= 'Du har lagt till ' . $user['username'] . ' i din vännerlista, <a href="?id=' . $_GET['id'] . '&action=removefriend">ta bort från vännerlistan</a>';
			$output .= rounded_corners_bottom(array('color' => 'blue_deluxe'), true);
		}
	...
	{
	$ui_options['title'] = $profile['username'] . 's foton, Hamsterpaj.net';
	}
